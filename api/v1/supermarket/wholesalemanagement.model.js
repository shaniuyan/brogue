...
exports.purchasingManagementListAsync = function (opts) {
    ...
    if (!opts.page.searchCount) {
        var findSqlStr = paramparse.parseFindSqlObjTotal(null, "wholesale_management");
    ...
    var findDataStr = paramparse.parseFindSqlObjLimit(obj, "wholesale_management", beginRowIndex, pageSize);
...
exports.purchasingManagementDetailListAsync = function (opts) {
    ...
    beginRowIndex = (pageIndex - 1) * pageSize;
    var findObj = {where: {wmId: opts.purchasingManagement.wmId}};
    var findDataStr = paramparse.parseFindSqlObjLimit(findObj, "wholesale_management_detail", beginRowIndex, pageSize);
...
exports.addPurchasingManagementAsync = function (opts) {
    ...
    var mysqlPool = opts.mysqldbs.mysqlPool;
    var insertObj = {
        whcode: opts.purchasingManagement.whcode,
    ...
    var findSqlStr = paramparse.parseFindSqlObjs(findPm, "wholesale_management");
    return mysqlPool.queryAsync(findSqlStr).then(function (result) {
        if (!result.length) {
            return getNextPurchasingManagementAsync(opts).then(function (result) {
                var tableName = "wholesale_management";
                insertObj.whcode = result.data;
                ...
                return mysqlPool.queryAsync(sqlObj.sqlStr, sqlObj.insertInfos).then(function (result) {
                    if (result.affectedRows == 1) {
                        insertObj.wmId = result.insertId;
                    }
                    results.error_code = 0;
...
exports.paymentOperationAsync = function (opts) {
    ...
    var mysqlPool = opts.mysqldbs.mysqlPool;
    var findPm = {
        where: {wmId: opts.purchasingManagement.wmId}
    };
    var tableName = "wholesale_management";
    ...
    return mysqlPool.queryAsync(findSqlStr).then(function (purchasings) {
        ...
        var updObj = {
            ...
            where: {
                wmId: opts.purchasingManagement.wmId
            }
        };
        ...
        var updateSql = paramparse.parseUpdateSqlObj(updObj, "wholesale_management");
...
exports.addPurchasingGoodsAsync = function (opts) {
    ...
    return join(findGoodAsync, findPurchasingManagementAsync, function (good, purchasing) {
        ...
        var tableName = "wholesale_management_detail";
        var sqlObj = paramparse.parseInsertSqlObj(insertObj, tableName);
        ...
        return join(addPmDetailAsync, updGoodNumAsync, updPurchasingManagementAsync, function (adddetail, goodNum, purchasingManagement) {
            ...
            if (result.adddetail.affectedRows == 1) {
                insertObj.wmDetailId = result.adddetail.insertId;
            }


            var findDetail = {
                where: {wmDetailId: result.adddetail.insertId}
            };

            var findDetailStr = paramparse.parseFindSqlObj(findDetail, "wholesale_management_detail");
...
exports.delPurchasingGoodsAsync = function (opts) {
    ...
    var findpmStr = paramparse.parseFindSqlObj(findPm, "wholesale_management");
    ...
    return mysqlPool.queryAsync(findpmStr).then(function (result) {
        ...
        var findPmDetailWhere = {
            where: {wmDetailId: opts.purchasingManagement.wmDetailId}
        };
        var findpmDetailStr = paramparse.parseFindSqlObj(findPmDetailWhere, "wholesale_management_detail");
        return mysqlPool.queryAsync(findpmDetailStr).then(function (result) {
            ...
            var deletePmDetailStr = paramparse.deleteFindSqlObj(findPmDetailWhere, "wholesale_management_detail");
            var updWhere = {
                set: {
                    totalprice: {
                        relationship: "-",
                ...
                where: {
                    wmId: findpmDetail.wmId
                }
            };
            var updPmPriceStr = paramparse.parseUpdateSqlObj(updWhere, "wholesale_management");
...
var calcPurchasingManagementPriceAsync = exports.calcPurchasingManagementPriceAsync = function (opts) {
    ...
    var findSql = paramparse.parseFindFieldSqlObj(findPurchasingManagementDetailObj, "wholesale_management_detail");
    return mysqlPool.queryAsync(findSql).then(function (result) {
        ...
        var updSql = paramparse.parseUpdateSqlObj(findPurchasingManagementDetailObj, "wholesale_management");
...
var getNextPurchasingManagementAsync = exports.getNextPurchasingManagementAsync = function (opts) {
    //select max(wholesalenum) whcode from brogue_db.wholesales;
    var results = {error_code: -1, error_msg: "error"};
    var bbPromise = opts.mysqldbs.bbpromise;
    var mysqlPool = opts.mysqldbs.mysqlPool;
    return mysqlPool.queryAsync("select max(whcode) whcode from brogue_db.wholesale_management").then(function (result) {
        results.error_code = 0;
        results.error_msg = "获取服务器最新采购编号成功!";
        if (!result[0].whcode) {
            result[0].whcode = 'WS' + new moment().format("YYYYMMDD") + "00001"
        } else {
            result[0].whcode = paramparse.nextBatchNumber(result[0].whcode);
        }
        results.data = result[0].whcode;
        return results;
    }).catch(function (e) {
        results.error_code = 1001;
