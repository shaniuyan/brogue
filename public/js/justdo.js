...
(function() {
    window.IM = window.IM || {
        ...
        _3rdServer : 'http://123.57.230.158:8886/authen/', // 3rdServer，主要用来虚拟用户服务器获取SIG
         ...
         /*
        //联调
        _appid : 'ff8080814f62be78014f63b20f870000', // 应用ID
        //_appid : 'ff8080814e760cd0014e760e70d00000', // 应用ID  小军的
        _onUnitAccount : 'KF10089', // 多渠道客服帐号，目前只支持1个
        _3rdServer : 'http://192.168.21.49:8070/authen/', // 3rdServer，主要用来虚拟用户服务器获取SIG
        */
        ...
        init : function() {
            // var a =
            // hex_md5('8a48b5514ef922c2014efcb7dcd903fa'+'18560033059'+'20150817094858'+'df200b2dd16e7b7aadc34ccf4cf15331');
            // console.log(a);
            ...
            // return;
            // }
            console.log("_getFileSig");
            var data = {
                "appid" : IM._appid
            };
            var url = IM._3rdServer+'genFileSig';
            $.ajax({
                url : url,
                dataType : 'jsonp',
                data : data,
        ...
        initSDK : function(sig){
            // 初始化SDK
            var resp = RL_YTX.init(IM._appid,null,null,sig);
            if (!resp) {
                alert('SDK初始化错误');
                return;
            };
        ...
        initEvent : function() {
             ...
             */
            // pre内光标插入事件绑定
            // IM._pre_range = new TextareaEditor(
            // document.getElementById('im_send_content') );
            $('#im_send_content')
        ...
        EV_keyCode : function(event) {
            ...
            //console.log("IM._keyCode_1="+IM._keyCode_1+"；IM._keyCode_2="+IM._keyCode_2);
        ...
        DO_login : function() {
            ...
            var user_account = $('#navbar_user_account').val();
            ...
            $.ajax({
                        ...
                        success : function(result) {
                            if (result.code != 000000) {
                                ...
                                // "000001"无此appid信息; "000002"sig信息为空;
                                // "000003"此appid无对应apptoken; "000004"时间戳有误;
                                var resp = {};
                                resp.code = result.code;
                                resp.sig = result.sig;
                                callback(resp);
        ...
        /*  $('#im_content_list').append(msg.prop("outerHTML"));
            msg.remove();// 删掉原来的展示
            msg = $(document.getElementById(receiver+"_"+oldMsgid));*/
            if (1 == msgtype) {// 文本消息
            	...
            	var oFile = IM._failAudioMap[receiver + '_' + oldMsgid];
                if (IM._contact_type_m == contact_type) {
                    IM.EV_sendToDeskAttachMsg(oldMsgid, oFile,
                            msgtype, receiver,true);
                } else {
        ...
        EV_sendAttachMsg : function(oldMsgid, file, type, receiver,isresend) {
            ...
            var msgid = RL_YTX.sendMsg(obj, function(obj) {
                        ...
                        setTimeout(function() {
                                    ...
                                    IM._failAudioMap[receiver + '_' + msgid]=file;
                                }, 100)
            ...
            RL_YTX.setGroupMessageRule(builder, function() {
                ...
                if (isNotice == 2) {
                    str = '<a href="#" class="btn btn-primary" style="margin-left:10px;" >开启</a>'
                            + '<a href="#" class="btn" style="margin-left:10px;" onclick="IM.EV_groupPersonalization(\''
                            + groupId + '\',1)">关闭</a>';
                } else {
                    str = '<a href="#" class="btn" style="margin-right:10px;" style="margin-left:10px;" onclick="IM.EV_groupPersonalization(\''
                            + groupId
                            + '\',2)">开启</a>'
        ...
        DO_pop_show_help_GroupDetail : function(obj, groupId, target) {
            ...
            if (isowner || target == 1) {
                ...
                var str_add = '<tr>'
                        ...
                        + '<tr>'
                        + '<td>'
                        + '<span class="pull-left" style="width: 25%;"><a href="#" class="btn" style="font-size: 20px;" onclick="IM.HTML_showInviteArea(this)" >+</a></span>'
                        + '<span class="pull-left" style="width: 25%; display: none;">'
                        + '<a href="#" class="btn" onclick="IM.EV_inviteGroupMember(\''
                        + groupId
                        + '\','
                        + obj.permission
                        + ',\''
            ...
            if (obj.isNotice == 1) {
                str = '<a href="#" class="btn" style="margin-left:10px;" onclick="IM.EV_groupPersonalization(\''
                        + groupId
                        + '\',2)" >开启</a>'
        ...
        DO_push_createMsgDiv : function(obj) {
            ...
            if (0 == obj.mcmEvent) {// 0普通im消息
                ...
                if (1 == msgType || 0 == msgType) {
                    ...
                    // str = you_msgContent;
                    // 转换所有的html标记
                    // you_msgContent = you_msgContent.replace(/</g,
                    // '&lt;').replace(/>/g, '&gt;');
        ...
        DO_showFireMsg : function(id,msgtype){
            ...
            //调用sdk传给发送侧 msgDomain {"msgid":""}   msgType=21  zzx
        ...
        DO_pop_phone : function(you_sender, version,obj) {
            ...
            var str = '';
            var showHei = $("#lvjing").height();
            if(!!videoUrl){
            	  if(!IM.Check_video_html()){
          		    str = '<pre>您有一条视频消息,请用其他设备接收</pre>';
          	      }else{
          	    	var type = videoUrl.substring(videoUrl.lastIndexOf(".")+1);
                    
                    str= '<video controls="controls" height="'+showHei+'px" style="position:relative;top:-20px;left:0px;">'+
                             '<source src="'+videoUrl+'" type="video/'+type+'"></video></video>';
          	      }
            }else{
           ...
           /* if($("#carousels").find("img")){
                $("#carousels").find("img").removeAttr("max-width");
            }*/
        ...
        HTML_pushMsg_addHTML : function(msgtype, you_sender, version,
            ...
            if(you_msgContent.indexOf("fireMsg") > -1){//fireMsg="yes"
                ...
                var fireMsgStr = '<img style="max-width:'+imgWid+'px; max-height:'+imgHei+'px; margin-right:5px; margin-left:5px;" ' +
        ...
        EV_getUserState : function(contact_you){
            ...
            RL_YTX.getUserState(getUserStateBuilder,function(obj){
                if(obj.state == 1){//1在线 2离线
                    onlineState.html("在线");
                    onlineState.show();
                    //onlineState.addClass("badge-success");
        ...
        DO_addContactToList : function() {
            ...
            if (im_contact.length <= 0) {
                ...
                //IM.HTML_clean_im_content_list(contactVal);//删 怀疑重复zzx
            }
        ...
        HTML_addContactToList : function(contactVal, contactName, content_type,
            ...
            $('#im_contact_list').prepend(str);
             ...
             // $('#im_contact_list li:first').css("width",liWidth+"px");
        ...
        DO_html_create : function(groupName,permission) {
            ...
            str += '</table></div></td></tr></table></div>'
                ...
                + '<a href="#" class="btn" onclick="IM.HTML_pop_hide()">取消</a>'
                + '</div>'
                + '</div>';
            ...
            for (var i in obj) {
                ...
                if (member.role == 1 || member.role == 2) {
                    ...
                    str += '<tr contact_you="'
                            ...
                            + '<a href="#" onclick="IM.editNickName(this)" '
                            + ' style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">'
                            + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this);" /></span></td>' + '</tr>';
                    ...
                    str += '<tr contact_you="'
                            ...
                            + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this);" /></span>';
                    if (isowner) {
        ...
        HTML_popAddMember : function(groupId, memberId, memberName, isowner,
            ...
            var str = '<tr contact_you="'
                    ...
                    + '<a href="#"  onclick="IM.editNickName(this)" ' 
                    + 'style="max-width:50%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">'
                    + memberName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.checkNick(this);" /></span>';
            if (isowner && isowner == 'true') {
                str += '<span class="pull-right label label-warning" onclick="IM.EV_deleteGroupMember(\''
        ...
        editNickName : function(obj){
            $(obj).hide();
            $(obj).next().show();
        },
        modifyName : function(obj){
        ...
        DO_pop_show : function(groupId, isowner, target) {
            ...
            str += '<div class="pull-right" style="width: 75%;" imtype="im_pop_group_name">'
                    ...
                    + '</span>' + '</td>' + '</tr>' + '</table>';
            // if(isowner){
            // str += '<p style="text-align: center;"><button class="btn
            // btn-primary" type="button" onclick="IM.EV_dismissGroup(\''+
            // groupId +'\')">解散群组</button></p>';
            // }
            if (target == 2) {
                if (!isowner) {
                ...
                if (isowner) {
                    str += '<div class="modal-footer">'
                            + '<a href="#" class="btn" onclick="IM.EV_dismissGroup(\''
                            + groupId
                            + '\')"> 解散群组 </a>'
                            + '<a href="#" class="btn btn-primary" onclick="IM.EV_updateGroupInfo(\''
                            + groupId + '\')">保存修改</a>' + '</div>';
                }
            }
            if (target == 1) {
                str += '<div class="modal-footer">'
                        + '<a href="#" class="btn btn-primary" onclick="IM.EV_updateGroupInfo(\''
                        + groupId + '\')">保存修改</a>'
                        + '<a href="#" class="btn" onclick="IM.EV_quitGroup(\''
                        + groupId + '\')">退出讨论组</a>' + '</div>';
            }
            str += '</div>';
        ...
        DO_sendMsg : function() {
            ...
            // 转换所有的html标记
            // im_send_content = im_send_content.replace(/&lt;/g,
            // '<').replace(/&gt;/g, '>');
        ...
        DO_im_attachment_file : function() {
            ...
            var str = '<div class="progress progress-striped active">'
                    ...
                    + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />'
                    + '</span>'
                    + '<span imtype="msg_attach_name"></span><span style="font-size: small;margin-left:15px;"></span>'
                    + '</a>'
        ...
        DO_im_attachment_file_up : function(id, oldMsgid) {
            // var oFile =
            // document.getElementById('im_attachment_file').files[0];
            var msg = $(document.getElementById(id));
            ...
            if(!!document.getElementById("image_button")){
                msg.find('a[imtype="msg_attach_href"]').attr('href', url);
                msg.find('span[imtype="msg_attach_name"]').html(oFile.name);
                msg.find('span[imtype="msg_attach_name"]').next().html(size);
                msgType = 6;
            }else{
	            if("image/jpeg" == oFile.type){
                    var windowWid = $(window).width();
		            var imgWid = 0;
		            var imgHei = 0;
		            if (windowWid < 666) {
        ...
        DO_chooseEmoji : function(unified, unicode) {
            ...
            // var content_emoji = '<span style="cursor:pointer;"
            // imtype="content_emoji">'+ out +'&nbsp;</span>';
            ...
            if ($('#im_send_content').children().length <= 1) {
                ...
                // $('#im_send_content').html('');
                // }
                // if( '<br>' == content.substr(content.length-4,
                // content.length) ){
                // //$('#im_send_content').html('');
                // }
                // if( '<div>' == content.substr(content.length-5,
                // content.length) ){
                // $('#im_send_content').html('');
                // }
            ...
            // IM._pre_range_num = IM._pre_range.insertData(content_emoji,
            // IM._pre_range_num);
        ...
        DO_userpop_show : function() {
            var str = '<div class="modal" id="pop_MyInfo" style="position: relative; top: auto; left: auto; right: auto; margin: 0 auto 20px; z-index: 1; max-width: 100%;">'
                    ...
                    + '</table>'
                    + '<div class="modal-footer">'
                    + '<a href="#" class="btn btn-primary" onclick="IM.EV_updateMyInfo()"> 保存修改 </a>'
                    + '<a href="#" class="btn" onclick="IM.HTML_pop_hide();">取消</a>'
                    + '</div></div>';
        ...
        DO_takePicture : function() {
            ...
            $("#takePhoto").css("height",$("#takePhoto").css("height").split("px")[0]- 50);
        ...
        _snapshot : function() {
            ...
            if("174010" == resultObj.code){//没有调用applay方法
                ...
                var str1 = '<div class="progress progress-striped active">'
                        ...
                        + '</div>'
                        + '<span imtype="msg_attach">'
                        + '<img imtype="msg_attach_src" src="'+url+'" onclick="IM.DO_pop_phone(\''+content_you+'\', \''+''+'\',this)" style="max-width:'+ imgWid + 'px;max-height:' + imgHei + 'px;" />'
        ...
        DO_getHistoryMessage : function() {
            ...
            if (scrollTop == 0) {
                // 获取参数
                // var firstMsg=content_list.find("div")[0];
                var firstMsg = null;
        ...
        EV_getHistoryMessage : function(firstMsg){
            ...
            RL_YTX.getHistoryMessage(getHistoryMessageBuilder,
                    function(obj) {
                        ...
                        if (obj.code == "540016") {
                            $("#getHistoryMsgDiv").html('<a href="#" style="font-size: small;position: relative;top: -30px;">没有更多历史消息</a>');
                        } else {
                            alert("错误码："+obj.code+"; 错误描述："+obj.msg)
                        }
                    ...
                    if(type == 1){
                        ...
                        if(!document.getElementById("cancelVoipCall")){
                            var str = '<div style="width:100%;height:30px;background-color:black;margin-top:-7px;' +
                                    'padding-top:5px;padding-bottom:5px;"><a href="#" id="cancelVoipCall" ' +
                                    'class="btn" >取消</a><a href="#" id="acceptVoipCall" ' +
                                    'class="btn btn-primary" >接受视频</a><a href="#" id="refuseVoipCall" ' +
                        ...
                        if(!document.getElementById("cancelVoiceCall")){
                            var audiostr = '<div style="width:100%;height:30px;background-color:black;margin-top:-7px;' +
                                    'padding-top:10px;background:transparent;"><a href="#" id="cancelVoiceCall" ' +
                                    'class="btn " >取消</a><a href="#" id="acceptVoiceCall" ' +
                                    'class="btn btn-primary" >接受音频</a><a href="#" id="refuseVoiceCall" ' +
       ...
       _sendRecorder : function (blob) {
            ...
            var str1 = '<div class="progress progress-striped active">'
                    ...
                    + '<input imtype="msg_attach_blobVoice" style="display:none;" value="'+blob.url+'"></input>'
                    + '</span>';
	        var msgid = new Date().getTime();
            var content_type = '';
            var content_you = '';
  ...
  SendFile_isDisable:function(){
	  $("#file_button").append(IM._forbidDiv);
	  $("#im_attachment_file").removeAttr("onclick");
	  
	  $("#image_button").append(IM._forbidDiv);
	  $("#im_image_file").removeAttr("onclick");
  },
  ...
  SendVoiceAndVideo_isDisable:function(){
	  $("#voipInvite").append(IM._forbidDiv);
	  $("#voipInvite").removeAttr("onclick");
	  
	  $("#voiceInvite").append(IM._forbidDiv);
  ...
  Check_usermedie_isDisable:function(){
    ...
    $("#camera_button").html('<label for="im_image_file" id="image_button" ><i class="icon-picture"></i></label>');
    
	$("#startRecorder").append(IM._forbidDiv);
   ...
   * 判断是否支持video标签
   */
  Check_video_html:function(){
	  var isVideo = !!(document.createElement('video').canPlayType);
	  return isVideo;
  },
  /**
   * 判断是否支持audio标签
   */
  Check_audio_html:function(){
